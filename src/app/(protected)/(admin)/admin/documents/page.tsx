...
import DocumentAction from "@/components/admin/DocumentAction";
...
const Documents = () => {
  ...
  const { data } = useSWR(
    `${process.env.NEXT_URL}/api/admin/document?page=${page}&limit=${limit}`,
  ...
  const fields = data?.documents?.map((p) => {
    return {
      ...
      status: p.status,
      date: p.createdAt,
      action: <DocumentAction documentId={p.id} />,
    };
  });
